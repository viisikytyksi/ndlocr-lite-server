...
対応するPARSEQ動的モデルの入力batch次元は動的で、FP32/FP16モデルをそれぞれ収録しています。

| モデル | FP32 | FP16 |
|---|---|---|
| `parseq-ndl-16x256-30` | 動的batch | 動的batch |
| `parseq-ndl-16x384-50` | 動的batch | 動的batch |
| `parseq-ndl-16x768-100` | 動的batch | 動的batch |

**PARSEQマイクロベンチマーク（256×16行画像16枚）**

| 環境 | モード | 16行あたり | 1行あたり | CPU逐次比 |
|---|---|---:|---:|---:|
| CPU | 逐次処理 FP32 | 337.6 ms | 21.1 ms | 1.00× |
| CPU | バッチ推論 FP32（batch=16） | 291.8 ms | 18.2 ms | 1.16× |
| AMD / MIGraphX | 逐次処理 FP16 | 111.3 ms | 7.0 ms | 3.03× |
| AMD / MIGraphX | バッチ推論 FP16（batch=16） | 13.9 ms | 0.9 ms | 24.29× |

AMD/MIGraphX FP16は、今回のbatch=16測定ではCPU逐次処理の約24.3倍、逐次処理でも約3.0倍でした。各モードの測定前に個別ウォームアップを行い、MIGraphXの遅延コンパイル・初回メモリ確保時間を測定値から除外しています。測定はPARSEQ行認識のマイクロベンチマークであり、DEIM検出・PDF変換・HTTP処理を含むPDF全体の処理時間ではありません。
