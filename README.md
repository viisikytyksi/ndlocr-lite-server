...
LLMserver上で、同一のPARSEQ行画像ベンチマークを実行した実測値です。PDF全体の処理時間ではありません。

| 実行環境 | 処理速度 |
|---|---:|
| CPU | 28.11 lines/s |
| AMD / MIGraphX | 83.99 lines/s |

この測定条件では、MIGraphXがCPUの約3.0倍でした。別のGPU・モデル・バッチサイズ・ドライバーで同じ値になることは保証しません。
